...
<div class="toggle-api-option-1 toggle-item <?php if ($displayOption == "0") {
    ...
    <!-- Search Term -->
    <p>
        <label for="<?php echo $this->get_field_id('term'); ?>"><?php _e('Search Term:'); ?><img src="<?php echo YELP_WIDGET_PRO_URL.'/includes/images/help.png'?>" title="<?php _e('The term you would like to display results for, ie: \'Bars\', \'Daycare\', \'Restaurants\'.', 'ywp'); ?>" class="tooltip-info" width="16" height="16" /></label>
    ...
    <!-- Location -->
    <p>
        <label for="<?php echo $this->get_field_id('location'); ?>"><?php _e('Location:'); ?><img src="<?php echo YELP_WIDGET_PRO_URL.'/includes/images/help.png'?>" title="<?php _e('The city name you would like to to search, ie \'San Diego\', \'New York\', \'Miami\'.', 'ywp'); ?>" class="tooltip-info" width="16" height="16" /></label>
    ...
    <p>
        ...
        <select name="<?php echo $this->get_field_name('limit'); ?>" id="<?php echo $this->get_field_id('limit'); ?>" class="widefat">
            ...
            foreach ($options as $option) { ?>
                ...
                <option value="<?php echo $option; ?>" id="<?php echo $option; ?>" <?php if($limit == $option || empty($limit) && $option == '4') { echo 'selected="selected"'; } ?>><?php echo $option; ?></option>
...
<div class="toggle-api-option-2 toggle-item  <?php if ($displayOption == "1") {
    ...
    <!-- Business ID -->
    <p>
        <label for="<?php echo $this->get_field_id('id'); ?>"><?php _e('Business ID:'); ?><img src="<?php echo YELP_WIDGET_PRO_URL.'/includes/images/help.png'?>" title="<?php _e('The Business ID is the portion of the Yelp url after the \'http://www.yelp.com/biz/\' portion. For example, the following business\'s URL on Yelp is \'http://www.yelp.com/biz/the-barbeque-pit-seattle-2\' and the Business ID is \'the-barbeque-pit-seattle-2\'. For more information <a href=\'http://wordimpress.com/yelps-apis-explained-for-use-with-yelp-widget-pro/#business-method\' target=\'_blank\'>Click Here</a>', 'ywp'); ?>" class="tooltip-info" width="16" height="16" /></label>
...
<div class="advanced-options toggle-item">
    ...
    <p>
        ...
        <label for="<?php echo $this->get_field_id('disable_title_output'); ?>"><?php _e('Disable Title Output', 'ywp'); ?><img src="<?php echo YELP_WIDGET_PRO_URL.'/includes/images/help.png'?>" title="<?php _e('The title output is content within the \'Widget Title\' field above. Disabling the title output may be useful for some themes.', 'ywp'); ?>" class="tooltip-info" width="16" height="16" /></label>
    </p>
    ...
    <p>
        ...
        <label for="<?php echo $this->get_field_id('target_blank'); ?>"><?php _e('Open Links in New Window', 'ywp'); ?><img src="<?php echo YELP_WIDGET_PRO_URL.'/includes/images/help.png'?>" title="<?php _e('This option will add target=\'_blank\' to the widget\'s links. This is useful to keep users on your website.', 'ywp'); ?>" class="tooltip-info" width="16" height="16" /></label>
    </p>
    <!-- No Follow Links -->
    <p>
        ...
        <label for="<?php echo $this->get_field_id('no_follow'); ?>"><?php _e('No Follow Links', 'ywp'); ?><img src="<?php echo YELP_WIDGET_PRO_URL.'/includes/images/help.png'?>" title="<?php _e('This option will add rel=\'nofollow\' to the widget\'s outgoing links. This option may be useful for SEO.', 'ywp'); ?>" class="tooltip-info" width="16" height="16" /></label>
    </p>

    <!-- Transient / Cache -->
    <p>
        <label for="<?php echo $this->get_field_id('cache'); ?>"><?php _e('Cache Data:'); ?><img src="<?php echo YELP_WIDGET_PRO_URL.'/includes/images/help.png'?>" title="<?php _e('Caching data will save Yelp data to your database in order to speed up response times and conserve API requests. The suggested settings is 1 Day. ', 'ywp'); ?>" class="tooltip-info" width="16" height="16" /></label>
        <select name="<?php echo $this->get_field_name('cache'); ?>" id="<?php echo $this->get_field_id('cache'); ?>" class="widefat">
            ...
            foreach ($options as $option) { ?>
                <option value="<?php echo $option; ?>" id="<?php echo $option; ?>" <?php if($cache == $option || empty($cache) && $option == '1 Day') { echo ' selected="selected" '; } ?>>
                <?php echo $option; ?>
                </option>
            <?php $counter++;
            }  ?>
...
<p>Upgrade to the <a href="http://wordimpress.com/wordpress-plugin-development/yelp-widget-pro/" target="_blank" class="new-window" title="<?php _e('Get immediate access after purchase to additional features, priority support and lifetime updates.','ywp'); ?>"><?php _e('Premium Version','ywp'); ?></a></p>
</div>
